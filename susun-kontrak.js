...
function spkEnsureHlStyle(){
  ...
  var css=
    ...
    '.spk-hlwrap .spk-hl-backdrop,.spk-hlwrap .spk-hl-input{'+
      "font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.55;"+
...
/* K = TINGGI BARIS TUNGGAL font dokumen (em) = (ascent + descent + lineGap) / unitsPerEm.
   FONT ISI KONTRAK = ARIAL (ketentuan 23 Jul 2026, menggantikan Inter). Metrik resmi
   Arial: upm 2048, ascent 1854, descent 434, lineGap 67 -> (1854+434+67)/2048 = 1,15.
   NILAI DIKUNCI, tidak diukur saat runtime. "1,15 baris" Word = 1,15 x 1,15 = 1,3225
   line-height CSS. Arial adalah font BAWAAN SISTEM (Windows/macOS/Office), jadi tidak
   perlu diunduh, tidak perlu @font-face, dan tidak pernah gagal muat — termasuk saat
   OFFLINE. Itulah alasan Inter (yang harus ditanam base64 / diambil dari Google Fonts)
   ditinggalkan. */
const SPK_LH_K = 1.15;                                   /* tinggi baris tunggal Arial (em) — TETAP */
const SPK_DOC_FONT = 'Arial,Helvetica,sans-serif';
...
/* Diagnostik jarak baris. Sejak font isi kontrak memakai ARIAL (font sistem) tidak ada
   lagi berkas font yang bisa gagal muat, jadi pemeriksaan @font-face dihapus; yang
   tersisa hanya pemeriksaan K (tinggi baris tunggal) supaya paginasi tetap terjaga. */
function spkCekFontDok(){ try{ spkCekFontK(); }catch(e){} }
function spkCekFontK(){
  try{
    var k = spkFontK(SPK_DOC_FONT,'11pt');
    if(k && Math.abs(k - SPK_LH_K) > 0.02){
      console.warn('[SPK] K terukur ('+k+') menyimpang dari K Arial resmi ('+SPK_LH_K+'). '+
        'Font isi kontrak mungkin bukan Arial. Jarak baris tetap memakai '+SPK_LH_K+'.');
    }
  }catch(e){}
}
try{
...
/* FONT ISI KONTRAK = ARIAL (23 Jul 2026).
   Arial terpasang bawaan di Windows, macOS, dan Microsoft Office, sehingga:
   - TIDAK butuh koneksi internet / Google Fonts,
   - TIDAK butuh berkas @font-face yang ditanam base64,
   - tampil sama di pratinjau, hasil Cetak/PDF, dan di Microsoft Word.
   Fungsi ini dipertahankan (mengembalikan string kosong) supaya pemanggilnya di
   spkDocCss() tidak perlu diubah bila kelak font tertanam dibutuhkan lagi. */
function spkInterFontFace(){ return ''; }
/* =====================================================================
...
function spkDocCss(){
  ...
  /* Variabel dokumen: --spk-lh = "1,15 baris" gaya Word = 1,15 x K Arial 1,15 = 1,3225.
     --spk-kv-gap = jarak bawaan baris "Label : nilai" bila tidak diatur sendiri. */
  ':root{--spk-lh:'+spkLHCss(1.15)+';--spk-kv-gap:4pt}'+
  'body{font-family:Arial,Helvetica,sans-serif;color:#1c1c1c;margin:0;font-size:11.5px;line-height:var(--spk-lh,1.3915)}'+
  ...
  /* CATATAN 23 Jul 2026: aturan lama
       .spk-doc.spk-spk .spk-clause .spk-cl:has(.spk-wx){padding-left:<BASE>}
     DIHAPUS. Aturan itu mengembalikan BASE (0,75 cm) untuk klausul hasil impor
     Word karena isinya dulu dibiarkan apa adanya (mode WYSIWYG). Sejak SELURUH
     klausul SPK dirapikan spkPkIndentStd (lihat spkPkTidy), posisi kiri setiap
     paragraf ditentukan inline; padding wadah akan menggesernya dua kali. */
...
function spkDocCss2(){
  ...
  /* Judul "SURAT PERINTAH KERJA" + Nomor Kontrak (halaman 1 isi kontrak):
     WARNA HITAM, jenis huruf mengikuti ISI KONTRAK (Arial 11pt),
  ...
  '.spk-coverpk{font-family:Arial,Helvetica,sans-serif;color:#000;'+
  ...
  /* --- Daftar Isi --- */
  '.spk-tocpage.toc-pk{font-family:Arial,Helvetica,sans-serif;color:#000;position:relative}'+
  ...
  '.spk-judulpk{text-align:center;font-family:Arial,Helvetica,sans-serif;'+
  ...
  '.spk-bab{text-align:center;font-family:Arial,Helvetica,sans-serif;margin:0 0 12pt}'+
  ...
  '.spk-bab span{display:block;font-family:Arial,Helvetica,sans-serif;font-size:11pt;font-weight:700;color:#000;letter-spacing:.06em;margin-top:7px}'+
...
/* ===== KETENTUAN 23 Jul 2026 (khusus Surat Perintah Kerja) =====
   "Inden patokannya BUKAN nomor klausul, melainkan TEKS JUDUL klausul", dan
   "kalau penomoran menyentuh 2 digit, batas kiri nomor 2 digit sejajar dengan
   teks di atasnya — nomor 1..9 otomatis sedikit masuk ke dalam".
   Artinya: TEPI KIRI KOTAK NOMOR tiap tingkat diletakkan TEPAT di kolom teks
   baris di atasnya (judul klausul untuk tingkat-1, kolom teks induk untuk
   tingkat berikutnya). Karena penanda dirata-KANANkan di dalam kotaknya, nomor
   2 digit ("10.") mengisi kotak dari tepi kiri = sejajar teks di atasnya,
   sedangkan nomor 1 digit ("1.") tampil menjorok selebar satu angka — persis
   penomoran rata-kanan multilevel Word. Jadi jorokan tambahan = 0.
   Nilainya sengaja dijadikan KONSTANTA tersendiri supaya mudah ditala bila
   kelak diminta jorokan tambahan (mis. 0,15) tanpa menyentuh jalur PK. */
var SPK_SPK_KOL_LEAD = 0;               /* jorokan tambahan SPK thd kolom teks di atasnya (cm) */
...
/* PENGUKUR LEBAR KOTAK NOMOR.
   spkTextWidthCm() memakai '11pt Arial'; sejak isi kontrak juga dirender dengan
   ARIAL (23 Jul 2026) pengukur dan hasil render memakai font yang sama persis,
   sehingga kotak nomor tidak pernah kekecilan/kebesaran secara berarti.
   Seluruh digit Arial sama lebar, jadi pengukuran KANONIK (semua digit diganti '0')
   berlaku tepat untuk nomor 1 digit maupun 2 digit.
...
var SPK_PK_MEAS_FONT = '11pt Arial,Helvetica,sans-serif';
...
/* Arial adalah font SISTEM: kanvas pengukur memakai font yang sama persis dengan
   yang dirender, dan seluruh digit Arial sama lebar (tabular by design), sehingga
   pengukuran kanonik ('0') tepat. Kelonggaran kecil tetap dipakai sebagai jaminan
   pembulatan sub-piksel; kelebihan lebar TIDAK menambah jeda ke teks (penanda
   dirata-kanankan, jeda = padding tetap) — ia hanya menggeser tepi KIRI kotak. */
var SPK_PK_MEAS_PAD    = 1.06;   /* cadangan (font pengganti) */
var SPK_PK_MEAS_PAD_OK = 1.02;   /* Arial aktif: hampir persis */
...
function spkPkMeasPad(){
  try{
    if(document.fonts && document.fonts.check && document.fonts.check('11pt Arial'))
      return SPK_PK_MEAS_PAD_OK;
  }catch(e){}
  return SPK_PK_MEAS_PAD_OK;   /* Arial font sistem: selalu tersedia */
}
...
/* Baris "Label : nilai" yang berasal dari TABEL Word (spkWTblToHtml) membawa
   margin-left inline hasil pembacaan w:tblInd, dipetakan relatif dasar klausul:
       margin-left = (tblInd - BASE) / 566,93 cm
   Untuk tabel yang di Word menempel margin (tblInd 0) nilainya NEGATIF (-0,75 cm
   pada kisi SPK), sehingga blok Nama Pekerjaan / Lokasi tertarik keluar ke kiri dan
   tampak menempel batas kertas — bukan menjorok di bawah judul klausul.
   KETENTUAN 23 Jul 2026: blok itu harus SELALU inden di bawah teks judul klausul,
   jadi inden bawaan Word di dalamnya dinolkan dan posisi kiri sepenuhnya ditentukan
   oleh margin-left yang dipasang spkPkIndentStd pada blok pembungkusnya. */
function spkKvResetInner(el){
  try{
    if(!el) return;
    if(el.classList && el.classList.contains('spk-kv')){ el.style.paddingLeft='0cm'; el.style.textIndent='0cm'; }
    var kv=el.querySelectorAll ? el.querySelectorAll('.spk-kv') : [];
    for(var i=0;i<kv.length;i++){
      kv[i].style.marginLeft='0cm';
      kv[i].style.paddingLeft='0cm';
      kv[i].style.textIndent='0cm';
    }
  }catch(e){}
}
function spkPkIndentStd(html, opsi){
  ...
  try{
    ...
    if(introX>0){
      ...
      /* 23 Jul 2026: blok pengantar (paragraf pembuka & tabel/baris "Label : nilai"
         seperti Nama Pekerjaan / Lokasi pada Klausul 1 URAIAN PEKERJAAN) diluruskan
         TEPAT ke kolom TEKS JUDUL klausul — "selalu inden di bawah tulisan URAIAN
         PEKERJAAN". Dulu menjorok +0,15 sehingga tidak segaris dengan judulnya. */
      var tepiIntro=(judulX>0) ? Math.round((judulX+SPK_SPK_KOL_LEAD)*100)/100 : introX;
      for(var c0=0;c0<box.children.length;c0++){
        ...
        spkKvResetInner(ch);                                /* buang inden bawaan Word di dalamnya */
        if(ch.tagName==='P'){ ch.style.textIndent='0cm'; ch.style.paddingLeft='0cm'; }
        adaIntro=true;
      }
    ...
    /* 23 Jul 2026: dengan ATAU tanpa blok pengantar, tepi kiri kotak nomor
       tingkat-1 SELALU di kolom teks judul klausul — sehingga nomor 2 digit
       sejajar dengan teks di atasnya dan nomor 1 digit menjorok sendirinya. */
    if(adaIntro) LEAD=(judulX>0) ? Math.round((judulX+SPK_SPK_KOL_LEAD)*100)/100
    ...
    else if(judulX>0) LEAD=Math.round((judulX+SPK_SPK_KOL_LEAD)*100)/100;
    ...
    for(i=0;i<deret.length;i++){
      ...
      /* SPK (23 Jul 2026): tepi kiri kotak anak TEPAT di kolom teks induknya
         (SPK_SPK_KOL_LEAD = 0) — inden bertingkat tetap terlihat karena tiap
         tingkat bergeser selebar kotak nomor induknya, dan nomor 2 digit anak
         sejajar dengan teks induk di atasnya. PK tidak diubah. */
      var _leadAnak = (opsi && opsi.pk) ? (g2.angka ? SPK_PK_LEAD_ANGKA : SPK_PK_LEAD) : SPK_SPK_KOL_LEAD;
      if(g2.induk) g2.base=g2.induk.base + (g2.induk.Wb||g2.induk.W) + _leadAnak;
    ...
    try{
      ...
      for(ci=0;ci<cAll.length;ci++){
        ...
        if(lastKol>0 && ce.classList && (ce.classList.contains('spk-kv')||ce.classList.contains('spk-kvgrp'))){
          var _lk=(opsi && opsi.pk) ? SPK_PK_LEAD_JUDUL : SPK_SPK_KOL_LEAD;
          ce.style.marginLeft=Math.round((lastKol+_lk)*100)/100+'cm';
          spkKvResetInner(ce);
        }
      }
    }catch(eKv){}
...
function spkPkTidy(html, isPk){
  /* KETENTUAN 23 Jul 2026 ("khusus SPK, buatkan inden otomatis pada penomoran
     bertingkat"): bentuk Surat Perintah Kerja TIDAK LAGI dilewati walau isinya
     berasal dari berkas Word. Dulu spkPkDariWord() membuat fungsi ini keluar
     lebih awal (mode WYSIWYG "inden biar saya atur di Word"), sehingga klausul
     hasil unggahan .docx memakai inden bawaan Word dan berbeda sendiri dari
     klausul yang diketik di aplikasi — inilah sumber Klausul 1 yang tidak
     sejajar. Kini SEMUA klausul SPK melewati mesin perapian yang sama.
     Bentuk Perjanjian/Kontrak TIDAK diubah: mode WYSIWYG Word tetap berlaku
     di sana (sudah dikonfirmasi "mantap" oleh pengguna 22 Jul 2026). */
  if(!isPk){
    var _D0=(typeof spkDX==='function')?spkDX():null;
    var _jh0=(typeof SPK_JH_OVR!=='undefined' && SPK_JH_OVR>0) ? SPK_JH_OVR
             : (_D0?Math.round((_D0.JUDUL_HANG/566.929)*100)/100:0.65);
    return spkPkIndentStd(spkPkBoxMark(html), {intro:Math.round((_jh0+SPK_PK_LEAD)*100)/100, judul:_jh0});
  }
...
function spkDocHtml(data, klausul){
  ...
  return '<!DOCTYPE html><html lang="id"><head><meta charset="utf-8"><title>&#8203;</title>'+
    ...
    /* Font isi kontrak: ARIAL — font sistem, tidak perlu unduhan/jaringan. */
    '<style>'+
